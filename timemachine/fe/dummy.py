...
def flag_stable_dummy_ixns(
    ...
    for ij, params in zip(bond_idxs, bond_params):
        bond_kv[ordered_tuple(ij)] = params
    ...
    for (i, j, k), params in zip(angle_idxs, angle_params):
        b0 = ordered_tuple((i, j))
        b1 = ordered_tuple((j, k))
        if (b0 in bond_kv) and (b1 in bond_kv):
            # bonds are present
            b0_k, b0_l = bond_kv[b0]
            b1_k, b1_l = bond_kv[b1]
            if np.all([b0_k > min_bond_k, b1_k > min_bond_k, b0_l > min_bond_l, b1_l > min_bond_l]):
                bonds_okay = True
            else:
                bonds_okay = False
        else:
    ...
    for (i, j, k, l), params in zip(torsion_idxs, torsion_params):
        ...
        a0 = ordered_tuple((i, j, k))
        a1 = ordered_tuple((j, k, l))

        if (a0 in angle_kv) and (a1 in angle_kv):
            a0_k, a0_t = angle_kv[a0]
            a1_k, a1_t = angle_kv[a1]
            if np.all(
                [
                    a0_k > min_angle_k,
                    a1_k > min_angle_k,
                    np.abs(a0_t - np.pi) > min_angle_offset,
                    np.abs(a1_t - np.pi) > min_angle_offset,
                    np.abs(a0_t - 0.0) > min_angle_offset,
                    np.abs(a1_t - 0.0) > min_angle_offset,
                ]
            ):
                angles_okay = True
            else:
